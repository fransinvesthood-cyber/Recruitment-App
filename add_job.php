...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'Admin') {
        echo "<script>alert('Unauthorized access! Only admins can post jobs.'); window.location.href='dashboard.php';</script>";
        exit();
    }
...
if (empty($criteria_skills) && empty($criteria_qualifications) && $criteria_min_experience <= 0 
        && empty($criteria_province) && empty($criteria_city) 
        && $criteria_min_age <= 0 && $criteria_max_age <= 0) {
        echo "<script>alert('Please provide at least one screening criterion (skills, qualifications, experience, location, or age range).'); window.history.back();</script>";
        exit();
    }
...
// Qualifications handled separately in new table
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('Error inserting department.');</script>";
            exit();
        }
    }
    $stmt->close();
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('Error inserting company.');</script>";
            exit();
        }
    }
    $stmt->close();
    ...
    $stmt = $conn->prepare("INSERT INTO job_postings
        (admin_id, department_id, company_id, position, location, job_description, duties, requirements, skills, minimum_criteria, salary, deadline)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    $stmt->bind_param("iiisssssssss", $admin_id, $department_id, $company_id, $position, $location,
        $job_description, $duties, $requirements, $skills, $minimum_criteria, $salary, $deadline);

    if ($stmt->execute()) {
        $job_id = $stmt->insert_id;

        // Insert notification for the admin
        $notification_message = "New job posting created: " . htmlspecialchars($position);
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .submit-container { 
            text-align: center; /* CHANGED: Centered button */
            margin-top: 30px; 
        }
        .btn-submit {
            background: var(--primary); color: white; border: none;
            padding: 12px 60px; /* Increased padding for better center look */
...
<body>
        ...
        <main>
            ...
            <div class="welcome-section">
                <div class="welcome-content">
                    ...
                    <p>Create and publish job listings to attract qualified candidates</p>
                </div>
            </div>
                ...
                <form method="post" action="add_job.php" id="jobForm">
                    <div class="form-section">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="department_name" name="department_name" required placeholder="e.g., Engineering">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label for="company_name">Company Name *</label>
                                <input type="text" id="company_name" name="company_name" required placeholder="e.g., TechCorp Inc.">
                            </div>
                            <div class="form-group">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="text" id="salary" name="salary" required placeholder="e.g., R25 000 - R45 000">
                            </div>
                            <div class="form-group">
                                <label for="deadline">Application Deadline</label>
                                <input type="datetime-local" id="deadline" name="deadline" placeholder="Select deadline (optional)">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="job_description" name="job_description" required placeholder="Provide a compelling summary of the role..."></textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <textarea id="duties" name="duties" required placeholder="List the main duties and responsibilities..."></textarea>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <textarea id="requirements" name="requirements" required placeholder="List required qualifications, education, and experience..."></textarea>
                            </div>
                            <div class="form-group">
                                <label for="skills">Technical Skills *</label>
                                <textarea id="skills" name="skills" required placeholder="List required technical and soft skills..."></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <textarea id="criteria_skills" name="criteria_skills" placeholder="e.g., PHP, MySQL, Laravel"></textarea>
                            </div>
                            <div class="form-group">
                                <label for="criteria_qualifications">Required Qualifications (comma-separated)</label>
                                <textarea id="criteria_qualifications" name="criteria_qualifications" placeholder="e.g., BSc Computer Science, Diploma IT, Matric with Maths"></textarea>
                            </div>
                            <div class="form-group">
                                <label for="criteria_province">Province</label>
                                <input type="text" id="criteria_province" name="criteria_province" placeholder="e.g., Gauteng, Western Cape">
                            </div>
                            <div class="form-group">
                                <label for="criteria_city">City/Town</label>
                                <input type="text" id="criteria_city" name="criteria_city" placeholder="e.g., Johannesburg, Cape Town">
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label for="criteria_min_experience">Minimum Years Experience</label>
                                <input type="number" id="criteria_min_experience" name="criteria_min_experience" min="0" value="0">
                            </div>
                            <div class="form-group">
                                <label>Age Range</label>
                                <div style="display: flex; gap: 10px;">
                                    <input type="number" id="criteria_min_age" name="criteria_min_age" min="16" max="80" placeholder="Min" style="flex: 1;">
                                    <span>-</span>
                                    <input type="number" id="criteria_max_age" name="criteria_max_age" min="16" max="80" placeholder="Max" style="flex: 1;">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="submit-container">
                        <input type="submit" value="Publish Job Posting" id="submitBtn" class="btn-submit">
                    </div>
                </form>
            </div>
        </main>
    ...
    <div class="custom-confirm" id="confirmDialog">
        <div class="confirm-box">
            <h3><i class='bx bx-check-circle' style="color:var(--primary); font-size:32px;"></i><br>Confirm Job Posting</h3>
            <p>Are you sure you want to publish this job listing? It will be visible to candidates immediately.</p>
            <div class="confirm-buttons">
                <button class="confirm-btn confirm-no" id="confirmNo">Cancel</button>
                <button class="confirm-btn confirm-yes" id="confirmYes">Yes, Publish</button>
            </div>
        </div>
    </div>
