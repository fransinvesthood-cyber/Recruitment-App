...
$stmt = $conn->prepare("SELECT work_date, client_project, summary, challenges, support, notes FROM consultant_task_logs WHERE user_id = ? AND work_date BETWEEN ? AND ?");
$stmt->bind_param("iss", $user_id, $start_date_str, $end_date_str);
$stmt->execute();
$stmt->bind_result($work_date, $client_project, $summary, $challenges, $support, $notes);
...
<h3>Weekly Task Log Report (<?= $startDate->format('M d') ?> – <?= $endDate->format('M d') ?>)</h3>
    ...
    <tbody>
        ...
        for ($date = clone $startDate; $date <= $endDate; $date->modify('+1 day')) {
            ...
            foreach ($tasklogs as $tl) {
                if ($tl['work_date'] === $currentDate) {
                    $entry['status'] = 'Present';
                    $entry['project'] = $tl['client_project'];
                    $entry['summary'] = $tl['summary'];
                    $entry['challenges'] = $tl['challenges'];
                    $entry['support'] = $tl['support'];
                    $entry['notes'] = $tl['notes'];
                    $hasTasklog = true;
            ...
            if (!$hasTasklog && !$isHoliday && !$isWeekend) {
            ...
            if (!$hasTasklog && $entry['status'] === 'Absent') {
        ...
        <tr style="border-bottom:1px solid #ccc; <?= $rowStyle ?>">
            ...
            <td><?= htmlspecialchars($entry['challenges']) ?></td>
            <td><?= htmlspecialchars($entry['support']) ?></td>
...
<form method="POST" action="export_task_log.php" onsubmit="return validateForm(this)" style="margin-top:20px;">
...
function validateForm(form) {
    ...
    if (!signature) {
        alert("Please enter your name to digitally sign the task log before exporting.");
        return false;
    }
    return true;
}
